...
def preprocess_labels(labels, training_set_dir):
	for (dirpath, dirnames, filenames) in os.walk(training_set_dir + '/labels'):
		for filename in filenames:
			...
			shape_file = open(training_set_dir + '/labels/' + filename.replace(".txt", ".sha"), "wb")
			if shape_list[1] == 0:
			...
			for i in range(len(value_list)):
				index_list.append(0)
				index_list.append(i)
			index_file = open(training_set_dir + '/labels/' + filename.replace(".txt", ".ind"), "wb")
...
def image_label_list(training_set_dir):
	...
	# for (dirpath, dirnames, filenames) in os.walk('training_set/pad_images'):
	for (dirpath, dirnames, filenames) in os.walk(training_set_dir + '/images'):
		# image_name_list.extend(map(lambda x: 'training_set/pad_images/' + x, filenames))
		image_name_list.extend(map(lambda x: training_set_dir + '/images/' + x, filenames))
		label_value_name_list.extend(map(lambda x: training_set_dir + '/labels/' + x.replace('.png', '.dat'), filenames))
		label_shape_name_list.extend(map(lambda x: training_set_dir + '/labels/' + x.replace('.png', '.sha'), filenames))
		label_index_name_list.extend(map(lambda x: training_set_dir + '/labels/' + x.replace('.png', '.ind'), filenames))
		break
	return image_name_list, label_value_name_list, label_shape_name_list, label_index_name_list

def create_inputs(training_set_dir, input_channel, labels, dilations, kernel_height, kernel_width, min_height, min_width_pad):
	height = 161
	...
	preprocess_labels(labels, training_set_dir)

	image_name_list, label_value_name_list, label_shape_name_list, label_index_name_list = image_label_list(training_set_dir)
